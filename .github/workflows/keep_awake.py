...
รันอัตโนมัติผ่าน GitHub Actions (ดูไฟล์ .github/workflows/keep-awake.yml)
"""
import os
...
def main():
    with sync_playwright() as p:
        ...
        # ถ้าแอปหลับอยู่ จะมีปุ่มปลุก "Yes, get this app back up!" → กดมัน
        try:
            btn = page.get_by_text("get this app back up", exact=False)
            if btn.count() > 0:
                print("→ เจอปุ่มปลุกแอป กำลังกด...")
                btn.first.click()
                page.wait_for_timeout(60000)  # รอแอปบูต ~1 นาที
        except Exception as e:
            print("ไม่มีปุ่มปลุก (แอปน่าจะตื่นอยู่แล้ว):", e)

        page.wait_for_timeout(15000)  # อยู่ต่ออีกหน่อยให้ timer รีเซ็ต
